...
def render_and_save(motion_input, save_path, keep_imgs=False, fps=25, color="#F96706#FB8D43#FDB381", with_conf=False, draw_face=False):
    ...
    if motion.shape[-1]==2 or motion.shape[-1]==3:
        motion = np.transpose(motion, (1,2,0))   #(N,17,D) -> (17,D,N), D = 2 or3
    if motion.shape[1]==2 or with_conf: # 2D
...
#     motion: (17,3,N)
    ...
    for f in tqdm(range(vlen)):
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        #azim = f*360.0/vlen # 영상 전체동안 한 바퀴
        azim = f*180.0/vlen # 영상 전체동안 반 바퀴
        elev = 12.
