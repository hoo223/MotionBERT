...
def p_mpjpe(predicted, target):
    ...
    # Rotation
    H = np.matmul(X0.transpose(0, 2, 1), Y0) # M = BA^T
    U, s, Vt = np.linalg.svd(H)
    V = Vt.transpose(0, 2, 1)
    R = np.matmul(V, U.transpose(0, 2, 1)) #  R = UV^T
...
def mpjpe_for_each_joint(predicted, target):
    """
    Mean per-joint position error (i.e. mean Euclidean distance),
    often referred to as "Protocol #1" in many papers.
    """
...
# https://en.wikipedia.org/wiki/Procrustes_analysis
def p_mpjpe_for_each_joint(predicted, target):
    """
    Pose error: MPJPE after rigid alignment (scale, rotation, and translation),
    often referred to as "Protocol #2" in many papers.
    """
...
def loss_symmetry(x):
    limb_lens = get_limb_lens(x)
    # limb_lens[0] # Pelvis-R_Hip
    # limb_lens[1] # R_Hip-R_Knee
    # limb_lens[2] # R_Knee-R_Ankle
    # limb_lens[3] # Pelvis-L_Hip
    # limb_lens[4] # L_Hip-L_Knee
    # limb_lens[5] # L_Knee-L_Ankle
    
    # limb_lens[10] # Neck-L_Shoulder
    # limb_lens[11] # L_Shoulder-L_Elbow
    # limb_lens[12] # L_Elbow-L_Wrist
    # limb_lens[13] # Neck-R_Shoulder
    # limb_lens[14] # R_Shoulder-R_Elbow
    # limb_lens[15] # R_Elbow-R_Wrist

    symmetry_pair = [[0,3], [1,4], [2,5], [10,13], [11,14], [12,15]]
    loss = 0
    for i in range(len(symmetry_pair)):
        j, k = symmetry_pair[i]
        loss += nn.L1Loss()(limb_lens[:, j], limb_lens[:, k])
    return loss
