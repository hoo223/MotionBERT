...
class DataReaderKOOKMIN(object):
...
#       Only Testset HW is needed for denormalization
        ...
        test_hw = np.array([test_hw[split_id_test[i]] for i in range(len(split_id_test))])[:,0,:]  #test_hw[split_id_test][:,0,:]                      # (N, 2)
        return test_hw
...
#       data: (N, n_frames, 51) or data: (N, n_frames, 17, 3)        
        ...
        for idx, item in enumerate(data):
            res_w, res_h = 1080, 1920 #test_hw[idx]
