...
from lib.model.DHDSTformer import DHDSTformer, DHDSTformer2, DHDSTformer3, DHDSTformer_torso
...
def evaluate(args, model_pos, test_loader, datareader):
    print('INFO: Testing')
    results_all_pred_torso = []
    results_all_pred_lower_frame_R = []
    results_all_pred_upper_frame_R = []
    model_pos.eval()            
    with torch.no_grad():
        for batch_input, batch_gt in tqdm(test_loader):
            ...
            results_all_pred_torso.append(pred_torso.cpu().numpy())
            results_all_pred_lower_frame_R.append(pred_lower_frame_R.cpu().numpy())
            results_all_pred_upper_frame_R.append(pred_upper_frame_R.cpu().numpy())
            if args.test_run:
                break
    results_all_pred_torso = np.concatenate(results_all_pred_torso)
    results_all_pred_torso = datareader.denormalize(results_all_pred_torso)
    ...
    part_list = ['whole'] # ['whole', 'torso', 'arms', 'legs', 'pelvis', 'r_hip', 'l_hip', 'torso', 'neck', 'l_shoulder', 'r_shoulder', 'l_elbow', 'l_wrist', 'r_elbow', 'r_wrist', 'r_knee', 'r_ankle', 'l_knee', 'l_ankle']
    ...
    for idx in range(len(results_all_pred_torso)):
        ...
        gt = gt_clips[idx][:, [0, 1, 4, 7, 8, 9, 10, 11, 14], :] # (243, 9, 3)
        pred = results_all_pred_torso[idx]
    ...
    return e1, e2, results_all_pred_torso
...
def train_epoch(args, model_pos, train_loader, losses, optimizer, has_3d, has_gt):
    ...
    for idx, (batch_input, batch_gt) in tqdm(enumerate(train_loader)):    
        ...
        optimizer.zero_grad()
        if has_3d:
            loss_3d_torso = loss_mpjpe(pred_torso, batch_torso)
            ...
            loss_total = args.lambda_torso         * loss_3d_torso + \
                         args.lambda_upper_frame_R * loss_upper_frame_R + \
                         args.lambda_lower_frame_R * loss_lower_frame_R 
                         
            losses['loss_3d_torso'].update(loss_3d_torso.item(), batch_size)
            losses['loss_upper_frame_R'].update(loss_upper_frame_R.item(), batch_size)
            losses['loss_lower_frame_R'].update(loss_lower_frame_R.item(), batch_size)
...
def train_with_config(args, opts):
    ...
    if opts.pretrained:
        ...
        chk_filename = ''
    if args.model == 'DHDST':
        model_pos = DHDSTformer(chk_filename=chk_filename, args=args)
    elif args.model == 'DHDST2':
        model_pos = DHDSTformer2(chk_filename=chk_filename, args=args)
    elif args.model == 'DHDST3':
    ...
    if not opts.evaluate:        
        ...
        for epoch in range(st, args.epochs):
            ...
            start_time = time()
            losses = {}
            losses['loss_3d_torso'] = AverageMeter()
            losses['loss_upper_frame_R'] = AverageMeter()
            losses['loss_lower_frame_R'] = AverageMeter()
            ...
            if args.no_eval:
                ...
                train_writer.add_scalar('loss_3d_torso', losses['loss_3d_torso'].avg, epoch + 1)
                train_writer.add_scalar('loss_upper_frame_R', losses['loss_upper_frame_R'].avg, epoch + 1)
                train_writer.add_scalar('loss_lower_frame_R', losses['loss_lower_frame_R'].avg, epoch + 1)
