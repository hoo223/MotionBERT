...
def validate2(test_loader, model, criterion):
    model.eval()
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for idx, (batch_input, batch_gt) in tqdm(enumerate(test_loader)):
            ...
            # update metric
            losses.update(loss.item(), batch_size)
            acc1, acc2, acc3, acc4, acc5 = accuracy(output, batch_gt, topk=(1, 2, 3, 4, 5))
            top1.update(acc1[0], batch_size)
            top2.update(acc2[0], batch_size)
            top3.update(acc3[0], batch_size)
            top4.update(acc4[0], batch_size)
            top5.update(acc5[0], batch_size)
            ...
            if (idx+1) % opts.print_freq == 0:
                print('Test: [{0}/{1}]\t'
                      ...
                      'Acc@5 {top5.val:.3f} ({top5.avg:.3f})\t'.format(
                       idx, len(test_loader), batch_time=batch_time,
                       loss=losses, top1=top1, top2=top2, top3=top3, top4=top4, top5=top5))
    return losses.avg, top1.avg, top2.avg, top3.avg, top4.avg, top5.avg
...
def train_with_config(args, opts):
    ...
    fit3d_train = Fit3DAction(data_path=data_path, data_split='test', n_frames=args.clip_len, random_move=args.random_move, scale_range=args.scale_range_train)
    fit3d_val = Fit3DAction(data_path=data_path, data_split='test', n_frames=args.clip_len, random_move=False, scale_range=args.scale_range_test)

    train_loader = DataLoader(fit3d_train, **trainloader_params)
    test_loader = DataLoader(fit3d_val, **testloader_params)
    ...
    if opts.evaluate:
        test_loss, test_top1, test_top2, test_top3, test_top4, test_top5 = validate2(test_loader, model, criterion)
        print('Loss {loss:.4f} \t'
              'Acc@1 {top1:.3f} \t'
              'Acc@2 {top2:.3f} \t'
              'Acc@3 {top3:.3f} \t'
              'Acc@4 {top4:.3f} \t'
              'Acc@5 {top5:.3f} \t'.format(loss=test_loss, top1=test_top1, top2=test_top2, top3=test_top3, top4=test_top4, top5=test_top5))
