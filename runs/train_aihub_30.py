...
def train_epoch(args, model_pos, train_loader, losses, optimizer, has_3d, has_gt):
    ...
    for idx, (batch_input, batch_gt) in tqdm(enumerate(train_loader)):    
        ...
        if has_3d:
            ...
            loss_3d_scale = n_mpjpe(predicted_3d_pos, batch_gt) # Normalized MPJPE
            loss_3d_velocity = loss_velocity(predicted_3d_pos, batch_gt) # Mean per-joint velocity error
            loss_lv = loss_limb_var(predicted_3d_pos) # limb 길이의 분산과 평균 error
            ...
            losses['symmetry'].update(loss_sym.item(), batch_size)
...
def train_with_config(args, opts):
    ...
    if torch.cuda.is_available():
        model_backbone = nn.DataParallel(model_backbone, output_device=0)
    ...
    if not opts.evaluate:        
        ...
        for epoch in range(st, args.epochs):
            ...
            if args.no_eval:
                ...
                train_writer.add_scalar('loss_symmetry', losses['symmetry'].avg, epoch + 1)
